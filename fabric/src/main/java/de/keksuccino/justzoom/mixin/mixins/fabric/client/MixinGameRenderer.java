...
public class MixinGameRenderer {
    ...
    private void wrap_Screen_render_JustZoom(Screen instance, PoseStack pose, int mouseX, int mouseY, float partial, Operation<Void> original) {
        original.call(instance, pose, mouseX, mouseY, partial);
        TooltipHandler.tick();
        PostRenderTasks.getAllAndClear().forEach(task -> task.render(pose, mouseX, mouseY, partial));
    }
